import { IoIosCheckmark } from "react-icons/io";
import { USER_ID } from "../../services/core-providers-di";

const ChatMessage = ({ message }) => {
  const isSender = message?.author == USER_ID
  return (
    <div className={`${isSender ? "self-end" : "self-start"}`}>
            <div className={`${isSender ? 'bg-green-300 rounded-full rounded-tr' : 'bg-blue-300 rounded-full rounded-tl self-start'} flex items-center text-sm sm:text-base text-black/50 shadow-lg py-2 px-3`}>
              <IoIosCheckmark className="text-2xl" />
              <p>{message?.message_data}</p>
          </div>
    </div>
  );
};

export default ChatMessage;
